...
int driveMode = 0; // 0=Manual, 1=Auto, 2=Wall
int radarMap[181]; 
long currentDist = 0;
...
bool smartDelay(unsigned long ms) {
  ...
  while (millis() - start < ms) {
    server.handleClient(); 
    if (driveMode == 0) return true; // Break loop if Manual
    delay(1);
  }
  return false;
}
...
<body>
    <div class="card" id="mainCard">
        ...
        <div class="dev" id="devPanel">Ajeet Kumar</div>
    </div>
...
void runAutoMode() {
    ...
    scanner.write(90); if(smartDelay(50)) return;
    int dist = getDistance(); radarMap[90] = dist;

    if (dist < 30) {
        moveStop(); if(smartDelay(100)) return;
        moveBackward(); if(smartDelay(250)) return;
        moveStop();
        
        scanner.write(170); if(smartDelay(300)) return;
        int left = getDistance(); radarMap[170] = left;
        scanner.write(10); if(smartDelay(300)) return;
        int right = getDistance(); radarMap[10] = right;
        scanner.write(90);
        
        if (left > right) { turnLeft(); if(smartDelay(400)) return; } 
        else { turnRight(); if(smartDelay(400)) return; }
        moveStop();
    } else { moveForward(); }
}

void runWallFollow() {
    if (driveMode != 2) return;
    scanner.write(0); if(smartDelay(50)) return;
    int d = getDistance(); radarMap[0] = d; currentDist = d;
    if (d < 15) { turnLeft(); smartDelay(100); moveForward(); }
    else if (d > 45) { turnRight(); smartDelay(100); moveForward(); }
    else moveForward();
}

void handleAction() {
    if (!server.hasArg("dir")) return;
    char cmd = server.arg("dir")[0];
    if (driveMode == 0) { 
        if (cmd == 'F') moveForward(); if (cmd == 'B') moveBackward();
        if (cmd == 'L') turnLeft(); if (cmd == 'R') turnRight();
        if (cmd == 'S') moveStop();
    }
    server.send(200, "text/plain", "OK");
}
void handleSetMode() { if (server.hasArg("m")) { driveMode = server.arg("m").toInt(); moveStop(); } server.send(200, "text/plain", "OK"); }
void handleScan() { String json = "{"; json += "\"a\":" + String(scanner.read()) + ","; json += "\"d\":" + String(currentDist) + ","; json += "\"map\":["; for(int i=0; i<=180; i+=10) { json += String(radarMap[i]); if(i<180) json += ","; } json += "]}"; server.send(200, "application/json", json); }
