...
<body>
  <div class="card">
    ...
    <div class="live-list">
        <div id="list-header" style="font-weight:bold; color:#00d2ff; margin-bottom:8px;">✅ Today's Present List: 0 / %TOTAL%</div>
  ...
  <script>
    ...
    document.getElementById('attForm').onsubmit = function(e) {
      ...
      .then(res => res.text()).then(text => {
        ...
        if(text.includes("Success") || text.includes("Pehle se")) {
             document.getElementById('msg').style.color = "#00ff88";
             updateList();
             if(text.includes("Success")) sel.selectedIndex = 0;
        } else {
...
// ---------------- ADMIN PAGE HTML (FIXED CENTERING) ----------------
...
void setup() {
  ...
  server.on("/present", [](){
    ...
    if(LittleFS.exists("/attendance.csv")){
      ...
      while(f.available()){
        ...
        if(comma1 > 0) {
           if(!first) namesArray += ",";
           namesArray += "\"" + line.substring(0, comma1) + "\"";
           first = false;
           count++;
        }
      }
    ...
    json += "\"total\":" + String(totalStudents) + ",";
  ...
  server.on("/mark", [](){
    if (server.hasArg("name")) {
      ...
      if(LittleFS.exists("/attendance.csv")){
        ...
        while(r.available()){
          ...
          if(comma1 > 0 && comma2 > comma1) {
            String existingName = line.substring(0, comma1);
            String existingIP = line.substring(comma2+1);
            existingIP.trim();

            if(existingName == name) alreadyPresent = true;
            
            // STRICT PROXY CHECK
            if(existingIP == userIP && existingName != name) {
                ipUsed = true;
                userWhoUsedIP = existingName;
            }
          }
        }
        r.close();
      ...
      if(ipUsed) {
        server.send(200, "text/plain; charset=utf-8", "❌ Proxy Alert! This device is already used by " + userWhoUsedIP);
        return;
      }
